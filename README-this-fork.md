README-this-fork.md
Last modified: 2026-03-12 16:14
Mark Torrey
...
* Check java version: `java -version1` and `javac -version` should both be 17+.
...
* Check that it is running and geocoding in a browser or with `curl -v "http://address.goes.here/"`:
	* `http://localhost:8080/geoclient/v2/version`
	* `http://localhost:8080/geoclient/v2/address?houseNumber=280&street=Riverside+Drive&borough=Manhattan`
	* `http://localhost:8080/geoclient/v2/search?input=350+Fifth+Avenue+Manhattan` (this one does natural language searches)
